import { GetArtistRadioSource, ProcessorResult } from '@smarter/shared';
...
export class GetArtistRadioProcessor extends Processor<GetArtistRadioSource> {
  async process(
    jobResources: JobResources,
    data: ProcessorResult[] = [],
  ): Promise<ProcessorResult[]> {
    const { spotifyClient } = jobResources;
    const { artist: artistAddress } = this.config.params;

    const artistId = extractId(artistAddress, ARTIST_RESOURCE_NAME);

    try {
      const response = await spotifyClient?.getRecommendations({ seed_artists: [artistId] });
      ...
      throw new ProcessorError(`artist ${artistId} does not exists`, this.config);
    } catch (err) {
